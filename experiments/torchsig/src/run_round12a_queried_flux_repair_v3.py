#!/usr/bin/env python3
"""Round 12A-v3: queried signed-flux repair without TorchSig controls."""
...
def freeze(config_relative: str, output: Path) -> str:
    ...
    names = [
        ...
        "ROUND12A_V3_QUERIED_FLUX_REPAIR_PROTOCOL.md",
        "src/run_round12a_queried_flux_repair_v3.py",
...
def main() -> None:
    ...
    parser.add_argument(
        "--config", default="configs/round12a_queried_flux_repair_smoke_v3.json"
    )
    ...
    queried_frames: list[pd.DataFrame] = []
    for arm, method in [
        ("random", "random"),
        ("uncertainty", "uncertainty"),
        ("queried_flux", "blind_flux"),
    ]:
        ranking = rank_unlabeled(
            ...
            confirmation,
            method,
            stable_seed(smoke["master_seed"], "selector", arm),
        )
        ranking = attach_hidden_diagnostics(
            ranking, acquisition_panels, acquisition_probabilities, anchor, confirmation
        )
        queried = stratified_prefix(ranking, query_budget, len(cfg["classes"]))
        queried.insert(0, "arm", arm)
        queried_frames.append(queried)
        if arm == "queried_flux":
    ...
    queried_candidates = pd.concat(queried_frames, ignore_index=True, sort=False)
    ...
    values = {
        ...
        "query_budget_per_arm": query_budget,
        ...
        "all_deployable_query_counts_equal": bool(
            queried_candidates.groupby("arm").size().nunique() == 1
        ),
        "all_deployable_query_anchor_prediction_balanced": bool(
            all(
                queried_candidates[queried_candidates["arm"] == arm]["anchor_predicted_class"]
                .value_counts()
    ...
    checks = {
        ...
        "equal_query_counts": bool(values["all_deployable_query_counts_equal"]),
        "query_predicted_class_quota_balanced": bool(
            values["all_deployable_query_anchor_prediction_balanced"]
        ),
