#!/usr/bin/env python3
"""Frozen 45 m Covertype signed-boundary multi-seed pilot."""
...
PRIOR_ROOT = ROOT.parent / "work_covtype_boundary_flow_probe"
...
def prior_panel_files() -> list[Path]:
    return [PRIOR_ROOT / "data" / "prior_fixed_panels.npz",
            PRIOR_ROOT / "data" / "flow_panels.npz",
            PRIOR_ROOT / "data" / "accounting_panels.npz"]
...
def freeze() -> str:
    files = [ROOT / "config.json", ROOT / "PROTOCOL.md", ROOT / "run_pilot.py",
             ROOT / "data" / "known_used_indices.npy", ROOT / "data" / "block_panels.npz",
             ROOT / "data" / "panel_audit.json", PRIOR_ROOT / "run_boundary_flow.py"]
...
def run(expected_hash: str) -> None:
    ...
    summary = {"pretarget_sha256": expected_hash, "models": 30,
               ...
               "decision": "ADVANCE_TO_FORMAL" if all(checks.values()) else "STOP_OR_REDESIGN"}
