#!/usr/bin/env python3
"""Round 12A-v4: common-query signed-flux repair without controls."""
...
def freeze(config_relative: str, output: Path) -> str:
    ...
    names = [
        ...
        "ROUND12A_V4_COMMON_QUERY_REPAIR_PROTOCOL.md",
...
def make_figure(path_frame: pd.DataFrame, diagnostics: pd.DataFrame, output: Path) -> None:
    ...
    selector_order = ["random", "uncertainty", "queried_flux"]
...
def main() -> None:
    ...
    parser.add_argument(
        "--config", default="configs/round12a_common_query_repair_smoke_v4.json"
    )
    ...
    for arm in ["random", "uncertainty", "queried_flux", "oracle_u"]:
    ...
    selector_precision = selections[
        selections["arm"].isin(["random", "uncertainty", "queried_flux"])
    ...
    checks = {
        ...
        "queried_selector_has_true_incident_signal": values["queried_flux_selector_incident_precision"]
        >= gates["minimum_queried_flux_selector_incident_precision"],
        "queried_selector_beats_random": values["queried_flux_selector_precision_advantage_over_random"]
        >= gates["minimum_queried_flux_selector_precision_advantage_over_random"],
        "queried_flux_reduces_end_risk": values["queried_flux_mean_end_risk_reduction"]
        >= gates["minimum_queried_flux_mean_end_risk_reduction"],
        "queried_flux_reduces_incident_crossing": values["queried_flux_mean_incident_reduction"]
        >= gates["minimum_queried_flux_mean_incident_reduction"],
        "queried_flux_beats_random_end_risk": values["queried_flux_end_risk_advantage_over_random"]
        >= gates["minimum_queried_flux_end_risk_advantage_over_random"],
        "queried_flux_beats_uncertainty_end_risk": values["queried_flux_end_risk_advantage_over_uncertainty"]
        >= gates["minimum_queried_flux_end_risk_advantage_over_uncertainty"],
        "queried_flux_preserves_baseline_validation": values["queried_flux_validation_accuracy_loss"]
        <= gates["maximum_queried_flux_validation_accuracy_loss"],
        "queried_flux_does_not_raise_anchor_risk": values["queried_flux_mean_anchor_risk_increase"]
        <= gates["maximum_queried_flux_mean_anchor_risk_increase"],
    ...
    make_figure(
        path_frame,
        selections[selections["arm"].isin(["random", "uncertainty", "queried_flux"])],
