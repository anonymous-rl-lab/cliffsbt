#!/usr/bin/env python3
"""Round 12A-v2: predicted-class-stratified blind boundary-flux repair."""
...
def freeze(config_relative: str, output: Path) -> str:
    ...
    names = [
        ...
        "ROUND12A_V2_STRATIFIED_FLUX_REPAIR_PROTOCOL.md",
        "src/run_round12a_blind_flux_repair_v2.py",
...
def main() -> None:
    ...
    parser.add_argument(
        "--config", default="configs/round12a_blind_flux_repair_smoke_v2_stratified.json"
    )
    ...
    for method in ["random", "uncertainty", "blind_flux"]:
        ...
        ranking = stratified_prefix(ranking, budget, len(cfg["classes"]))
    ...
    values = {
        ...
        "all_blind_arms_anchor_prediction_balanced": bool(
            all(
                selections[selections["arm"] == arm]["anchor_predicted_class"]
                .value_counts()
                .reindex(range(len(cfg["classes"])), fill_value=0)
                .nunique()
                == 1
                for arm in ["random", "uncertainty", "blind_flux"]
            )
        ),
    ...
    checks = {
        ...
        "predicted_class_quota_balanced": bool(values["all_blind_arms_anchor_prediction_balanced"]),
