#!/usr/bin/env python3
"""Round 12B: counterfactual repair-influence smoke without controls."""
...
def freeze(config_relative: str, output: Path) -> str:
    ...
    names = [
        ...
        "ROUND12B_INFLUENCE_REPAIR_SMOKE_PROTOCOL.md",
        "src/run_round12b_influence_repair_smoke.py",
            ...
            rows.append(
                {
                    ...
                    "uncertainty_repair_time": uncertainty_time,
                }
            )
...
def attach_hidden_diagnostics(
    ...
    diagnostics: dict[str, tuple[np.ndarray, np.ndarray, np.ndarray]] = {}
    for path, panel in panels.items():
        ...
        diagnostics[path] = (panel["labels"], first, margin)
    result = ranking.copy()
    result["class_index"] = [
    ...
    for row in result.itertuples():
        margin = diagnostics[row.path][2][:, int(row.sample_index)]
        first = int(diagnostics[row.path][1][int(row.sample_index)])
        if first >= 0:
            repair_time = first
            persistence = float(np.mean(margin[first:] <= 0))
            incident = 1.0
        else:
            repair_time = int(anchor + 1 + np.argmin(margin[anchor + 1 :]))
            persistence = 0.0
            incident = 0.0
        descent = float(max(0.0, margin[anchor] - np.min(margin[anchor + 1 :])))
        negative_area = float(np.mean(np.maximum(0.0, -margin[anchor + 1 :])))
        true_scores.append(4.0 * incident + 2.0 * persistence + descent + negative_area)
        true_repair_times.append(repair_time)
    result["queried_true_flux_score"] = true_scores
    result["queried_true_repair_time"] = true_repair_times
    return result
...
def stratified_prefix(ranking: pd.DataFrame, budget: int, classes: int) -> pd.DataFrame:
    """Select equal quotas by an unlabeled anchor-prediction stratum."""
    if budget % classes != 0:
        raise ValueError("stratified smoke budget must divide evenly by class count")
    quota = budget // classes
    selected = []
    for predicted_class in range(classes):
        group = ranking[ranking["anchor_predicted_class"] == predicted_class].head(quota)
        if len(group) != quota:
            raise RuntimeError(f"insufficient candidates for predicted class {predicted_class}")
...
def true_class_balanced_prefix(
    queried: pd.DataFrame, budget: int, classes: int, score_column: str
) -> pd.DataFrame:
    """Allocate the training budget after labels have been queried."""
    if budget % classes != 0:
        raise ValueError("repair budget must divide evenly by class count")
    quota = budget // classes
    selected = []
    for true_class in range(classes):
        group = queried[queried["class_index"] == true_class].sort_values(
            [score_column, "path", "sample_index"], ascending=[False, True, True]
        ).head(quota)
        if len(group) != quota:
            raise RuntimeError(f"query panel lacks repair quota for true class {true_class}")
        selected.append(group)
...
def screening_objective(
    ...
    for panel in panels.values():
        ...
        metrics, _, _ = light_path_metrics(probabilities, panel["labels"], anchor, confirmation)
        values.append(
            float(weights["end_risk"]) * metrics["end_risk"]
            + float(weights["incident_crossing_fraction_all"])
            * metrics["incident_crossing_fraction_all"]
            + float(weights["risk_area"]) * metrics["risk_area"]
        )
...
def make_figure(path_frame: pd.DataFrame, diagnostics: pd.DataFrame, output: Path) -> None:
    ...
    selector_order = ["random", "uncertainty", "queried_flux", "influence_flux"]
...
def main() -> None:
    ...
    parser.add_argument(
        "--config", default="configs/round12b_influence_repair_smoke.json"
    )
    ...
    query_budget = int(round(len(base_labels) * float(smoke["repair"]["query_fraction"])))
    ...
    common_ranking = rank_unlabeled(
        acquisition_probabilities,
        anchor,
        confirmation,
        "random",
        stable_seed(smoke["master_seed"], "common_query_pool"),
    )
    common_ranking = attach_hidden_diagnostics(
        common_ranking, acquisition_panels, acquisition_probabilities, anchor, confirmation
    )
    common_query = stratified_prefix(common_ranking, query_budget, len(cfg["classes"]))

    screening_spec = {
        "master_seed": int(smoke["screening"]["master_seed"]),
        "paired_deployment": {"samples_per_path": int(smoke["screening"]["samples_per_path"])},
    }
    screening_panels, screening_hash = paired_panel(cfg, screening_spec)
    baseline_screening_objective = screening_objective(
        base_model,
        screening_panels,
        anchor,
        confirmation,
        smoke["screening"]["objective_weights"],
    )
    for arm in ["random", "uncertainty", "queried_flux"]:
        queried = common_query.copy()
        queried.insert(0, "arm", arm)
        if arm == "queried_flux":
            queried["chosen_time"] = queried["queried_true_repair_time"]
            score_column = "queried_true_flux_score"
        elif arm == "uncertainty":
            queried["chosen_time"] = queried["uncertainty_repair_time"]
            queried["selection_score"] = -queried["minimum_uncertainty_gap"]
            score_column = "selection_score"
        else:
            score_column = "score"
        selected = true_class_balanced_prefix(
            queried, budget, len(cfg["classes"]), score_column
        )
        features, labels, selected = selected_arrays(selected, acquisition_panels, budget)
        repair_arrays[arm] = (features, labels, selected)
        diagnostic_frames.append(selected)

    influence_candidates = common_query.copy()
    influence_candidates["chosen_time"] = influence_candidates["queried_true_repair_time"]
    influence_scores = []
    candidate_objectives = []
    for row in influence_candidates.itertuples():
        candidate_feature = acquisition_panels[row.path]["features"][
            int(row.chosen_time), int(row.sample_index)
        ][None, :]
        candidate_label = np.asarray([int(row.class_index)], dtype=int)
        candidate_training, candidate_labels, _ = replace_same_class(
            base_features, base_labels, candidate_feature, candidate_label, seed
        )
        candidate_model = fit_model(candidate_training, candidate_labels, cfg, seed)
        objective = screening_objective(
            candidate_model,
            screening_panels,
            anchor,
            confirmation,
        ...
        influence_scores.append(baseline_screening_objective - objective)
    influence_candidates["candidate_screening_objective"] = candidate_objectives
    influence_candidates["influence_score"] = influence_scores
    influence_candidates.insert(0, "arm", "influence_flux")
    influence_selected = true_class_balanced_prefix(
        influence_candidates, budget, len(cfg["classes"]), "influence_score"
    )
    influence_features, influence_labels, influence_selected = selected_arrays(
        influence_selected, acquisition_panels, budget
    )
    repair_arrays["influence_flux"] = (
        influence_features,
        influence_labels,
        influence_selected,
    )
    diagnostic_frames.append(influence_selected)
    ...
    for arm in ["random", "uncertainty", "queried_flux", "influence_flux", "oracle_u"]:
    ...
    queried_candidates = common_query.copy()
    selector_precision = selections[
        selections["arm"].isin(["random", "uncertainty", "queried_flux", "influence_flux"])
    ...
    queried_effect = effect_index.loc["queried_flux"]
    influence_effect = effect_index.loc["influence_flux"]
    ...
    values = {
        ...
        "common_query_budget": query_budget,
        ...
        "all_three_streams_disjoint": len({acquisition_hash, screening_hash, evaluation_hash}) == 3,
        "common_query_count_correct": len(queried_candidates) == query_budget,
        "common_query_anchor_prediction_balanced": bool(
            queried_candidates["anchor_predicted_class"]
            .value_counts()
            .reindex(range(len(cfg["classes"])), fill_value=0)
            .nunique()
            == 1
        ),
        "all_deployable_repair_true_class_balanced": bool(
            all(
                selections[selections["arm"] == arm]["class_index"]
                .value_counts()
                .reindex(range(len(cfg["classes"])), fill_value=0)
                .nunique()
                == 1
                for arm in ["random", "uncertainty", "queried_flux", "influence_flux"]
            )
        ),
        ...
        "queried_flux_selector_incident_precision": float(selector_precision["queried_flux"]),
        "queried_flux_selector_precision_advantage_over_random": float(
            selector_precision["queried_flux"] - selector_precision["random"]
        ),
        "queried_flux_mean_end_risk_reduction": float(queried_effect.mean_end_risk_reduction),
        "queried_flux_mean_incident_reduction": float(queried_effect.mean_incident_crossing_reduction),
        "queried_flux_end_risk_advantage_over_random": float(
            queried_effect.mean_end_risk_reduction - random_effect.mean_end_risk_reduction
        ),
        "queried_flux_end_risk_advantage_over_uncertainty": float(
            queried_effect.mean_end_risk_reduction - uncertainty_effect.mean_end_risk_reduction
        ),
        "queried_flux_validation_accuracy_loss": float(
            validation["baseline"] - validation["queried_flux"]
        ),
        "queried_flux_mean_anchor_risk_increase": float(queried_effect.mean_anchor_risk_change),
        "baseline_screening_objective": baseline_screening_objective,
        "mean_selected_candidate_influence_score": float(
            influence_selected["influence_score"].mean()
        ),
        "influence_flux_mean_end_risk_reduction": float(
            influence_effect.mean_end_risk_reduction
        ),
        "influence_flux_mean_incident_reduction": float(
            influence_effect.mean_incident_crossing_reduction
        ),
        "influence_flux_end_risk_advantage_over_random": float(
            influence_effect.mean_end_risk_reduction - random_effect.mean_end_risk_reduction
        ),
        "influence_flux_end_risk_advantage_over_uncertainty": float(
            influence_effect.mean_end_risk_reduction
            - uncertainty_effect.mean_end_risk_reduction
        ),
        "influence_flux_end_risk_advantage_over_queried_flux": float(
            influence_effect.mean_end_risk_reduction - queried_effect.mean_end_risk_reduction
        ),
        "influence_flux_validation_accuracy_loss": float(
            validation["baseline"] - validation["influence_flux"]
        ),
        "influence_flux_mean_anchor_risk_increase": float(
            influence_effect.mean_anchor_risk_change
        ),
    ...
    checks = {
        ...
        "disjoint_acquisition_screening_evaluation_streams": bool(
            values["all_three_streams_disjoint"]
        ),
        "common_query_count": bool(values["common_query_count_correct"]),
        "common_query_predicted_class_quota_balanced": bool(
            values["common_query_anchor_prediction_balanced"]
        ),
        "repair_true_class_quota_balanced": bool(
            values["all_deployable_repair_true_class_balanced"]
        ),
        "baseline_cliff_present": values["baseline_common_cliff_fraction"]
        >= gates["minimum_baseline_common_cliff_fraction"],
        "influence_flux_reduces_end_risk": values["influence_flux_mean_end_risk_reduction"]
        >= gates["minimum_influence_flux_mean_end_risk_reduction"],
        "influence_flux_reduces_incident_crossing": values["influence_flux_mean_incident_reduction"]
        >= gates["minimum_influence_flux_mean_incident_reduction"],
        "influence_flux_beats_random_end_risk": values["influence_flux_end_risk_advantage_over_random"]
        >= gates["minimum_influence_flux_end_risk_advantage_over_random"],
        "influence_flux_beats_uncertainty_end_risk": values["influence_flux_end_risk_advantage_over_uncertainty"]
        >= gates["minimum_influence_flux_end_risk_advantage_over_uncertainty"],
        "influence_flux_beats_queried_flux_end_risk": values[
            "influence_flux_end_risk_advantage_over_queried_flux"
        ]
        >= gates["minimum_influence_flux_end_risk_advantage_over_queried_flux"],
        "influence_flux_preserves_baseline_validation": values[
            "influence_flux_validation_accuracy_loss"
        ]
        <= gates["maximum_influence_flux_validation_accuracy_loss"],
        "influence_flux_does_not_raise_anchor_risk": values[
            "influence_flux_mean_anchor_risk_increase"
        ]
        <= gates["maximum_influence_flux_mean_anchor_risk_increase"],
    ...
    queried_candidates.to_csv(output / "queried_candidates.csv", index=False)
    influence_candidates.to_csv(output / "candidate_influence_scores.csv", index=False)
    ...
    summary = {
        ...
        "screening_panel_sha256": screening_hash,
    ...
    make_figure(
        ...
        selections[
            selections["arm"].isin(
                ["random", "uncertainty", "queried_flux", "influence_flux"]
            )
        ],
